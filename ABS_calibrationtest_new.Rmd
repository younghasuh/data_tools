...
beep_data$id <- paste(beep_data$TagId, beep_data$session_id) # I don't understand why this is  needed
...
test$pt <- tags$NodeID[match(test$freq, tags$session_id)] # the 4 nodes where the tests were done



matchloc <- test[!duplicated(test$freq),] # freq is the session ID?
testloc <- weighted_average(freq = freq[1], beep_data, node = nodes,  calibrate = "session_id", MAX_NODES = 0) #sp data
...
testloc$pt <- tags$NodeID[match(testloc$group, tags$session_id)] # testloc$group are the session IDs


table(testloc$pt) # these are the 4 nodes still 
table(testloc$group) # these are the session IDs 
...
#### Things to do before the for loop
## need to figure out what weighted_average is doing because it significantly reduces the data frame to a few points
testloc <- weighted_average(freq = freq[1], beep_data, node = nodes,  calibrate = "session_id", MAX_NODES = 0) #sp data
# table(testloc$TagId)
#6133331E 61335578 61336634 6134074B 61526633 61611E2D 6166194B 6166194C 61664B1E 61664C61 
#      23       24       24       24       24       24       12       24       12       24 
# why does it reduce the data so much? These are 1 minute intervals  

testloc2 <- weighted_average(freq = freq[1], beep_data, node = nodes)

#table(testloc2$TagId)
#6133331E 61335578 61336634 6134074B 61526633 61611E2D 6166194B 6166194C 61664B1E 61664C61 
#     345      365      361      338      360      365      183      361      161      365 
...
```{r}
# Make nodes into spatial data for plotting
nodes_spatial <- nodes
coordinates(nodes_spatial) <- 3:2
...
# set up data from beep data
freq <- c("1 min", "3 min", "10 min")

test <- advanced_resampled_stats(beep_data, nodes, freq = freq[1], keep_cols = c("TagLat", "TagLng"), calibrate = "session_id") #freq is now at 3 min
test$pt <- tags$NodeID[match(test$freq, tags$session_id)] # the 4 nodes where the tests were done
matchloc <- test[!duplicated(test$freq),] # freq is the session ID
...
# Create a function that will run this all at once
trials <- 1:61

# for loop works but the images are the same. :/ 
for (n in trials) {
  testloc <- weighted_average(freq = freq[1], beep_data, node = nodes,  calibrate = "session_id", MAX_NODES = 0) #sp data
  ...
  test_loc <- st_as_sf(testloc) # sp to sf data 
  
  test_loc_n <- test_loc[which(test_loc$group == 20),]
  
  myplot <- ggplot() + 
     geom_text(data = nodes, aes(x=lng, y=lat, label = NodeId), size = 5) +
     geom_sf(data = tag_loc, colour="red") + # doesn't even get plotted
     geom_sf(data = my_nodes, colour="blue") +
     geom_sf(data = test_loc_n, aes(colour=pt)) +
     labs(title = paste("experimental trial", n, sep = " "))
  myplot
   
  ggsave(filename = paste(n, "_trial.png", sep = ""), plot = myplot )
}

# only test_loc gets plotted not tag_loc
```
